...
try:
    ...
    for framestart in irange:
        ...
        for winstart in range(framestart, framestop):
            ...
            indx += 1
        # Pick smallest absolute difference in frame ->f0
        smallest = np.min(meandiffs[diffoffset:])
        # Get position of the smallest absolute difference -> f0
...
try:
    ...
    print('Output in CSV file. Open with "soffice %s", choose comma separator, click on the left of row 1, '
          'then Insert, then Chart, then format the chart as a scatter plot.' % csvfilename)
